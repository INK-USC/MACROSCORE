...
class Model:
    def __init__(self):
        ...
        self.bertmodel = LongformerModel.from_pretrained('allenai/longformer-base-4096')
        self.bertmodel.to('cuda')

        loadFilename = os.path.join('9_checkpoint.tar') #TODO
        if loadFilename:
        ...
        self.result = {
            "model_keywords": ["longformer", "meta_analysis_significant"],
            "model_name": "longformer_meta_analysis_significant_classifier",
            "model_author": "Dong-Ho",
            "model_timestamp": datetime.now().timestamp(),
            "tag": "meta_analysis_significanat_classifier_with_text"
        }

    def split_paper(self, content):
        l_total = []
        if len(content.split()) // 256 > 0:
            n = len(content.split()) // 256
        else:
            n = 1
        for w in range(n):
            if w == 0:
                l_parcial = content.split()[:512]
                l_total.append(" ".join(l_parcial))
            else:
                l_parcial = content.split()[w * 256:w * 256 + 512]
                l_total.append(" ".join(l_parcial))
        return l_total
    ...
    def data_loader_example(self):
        ...
        for k in meta['pdf_filename'].keys():
            ...
            if os.path.exists('../parsed_ta1/'+filename+'.parsed_rpp'):
                data = json.load(open('../parsed_ta1/'+filename+'.parsed_rpp', 'rb'))
            ...
            total += 1
        print(total, error)
        json.dump(self.data, open("../data_processed/TA1_scienceparse_classify_data.parsed_rpp", 'w'))
    ...
    def get_result(self) -> dict:
        ...
        if test == 'rpp':
            ...
            json.dump(self.result, open("../dongho_longformer_rpp.json", 'w'))
        elif test == 'ta1':
            ...
            result = {"result": [{"pid": pid, "score": str(score)} for pid, score in zip(pids, predictions)]}
            self.result.update(result)
            json.dump(self.result, open("../dongho_longformer_ta1.json", 'w'))
        elif test == 'covid':
            ...
            result = {"result": [{"pid": pid, "score": str(score)} for pid, score in zip(pids, predictions)]}
            self.result.update(result)
            json.dump(self.result, open("../dongho_longformer_covid.json", 'w'))
        else:
    ...
    def submit_result(self, endpoint="http://ckg03.isi.edu:7789/submission"):
        result = json.load(open('../dongho_scibert.parsed_rpp', 'r'))
