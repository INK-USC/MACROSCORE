...
def read_trees_from_corpus_repr(path):
    f = open(path)
    header = f.readline()
    reader = csv.reader(f)
    trees = []
    ...
    for i, line in enumerate(reader):
        cur_tree = parse_tree(line[-1].lower())
        trees.append(cur_tree)

        cur_dict = {}
        cur_dict["paper_id"] = line[0]
        cur_dict["label"] = line[1]
        cur_dict["important_segment"] = line[2]
        paper_info.append(cur_dict)
    return trees, paper_info
